...
import 'package:learn_english/view/play_game/features/slide_party/features/playboard/models/playboard_animation_types.dart';
...
class PlayboardView extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final animationTypes = ref.watch(playboardControllerProvider.select((value) {
      if (value is SinglePlayboardState) {
        if (value.playboard.isSolved) {
          return PlayboardAnimationTypes.values[Random().nextInt(PlayboardAnimationTypes.values.length)];
        }
      }
      return null;
    }));
    ...
    return IgnorePointer(
      ...
      child: RepaintBoundary(child: Center(child: _animatedPlayboard(animationTypes))),
    );
  }
      ...
      builder: (context, value, child) {
        return Transform.rotate(
          ...
          child: Transform.scale(
            scale: value == 0 ? 1 : 1 + (size / sqrt(size * size * 2) - 1) * value,
            child: _basePlayboard(PlayboardAnimationTypes.heartScaleFade, value),
          ),
        );
      },
    );
      ...
      builder: (context, ref, child) {
        final loc = ref.watch(
          playboardControllerProvider.select((state) {
            ...
            throw UnimplementedError('This kind of playboard is not implemented yet.');
          }),
        );
    ...
    if (index == boardSize * boardSize - 1) {
      return Container(
        ...
        child: SizedBox(
          ...
          height: (size - 44) / boardSize,
          width: (size - 44) / boardSize,
          child: holeWidget,
        ),
      );
    }
      ...
      switch (animationType) {
        ...
        case PlayboardAnimationTypes.spinTile:
          return Transform.rotate(
            ...
            origin: const Offset(0.5, 0.5),
            child: Transform.scale(
              scale: animateValue <= 0.4 ? 1 - animateValue / 2 : 0.8 + (animateValue) / 5,
              child: numberTile,
            ),
          );
        case PlayboardAnimationTypes.spinTileWithFade:
